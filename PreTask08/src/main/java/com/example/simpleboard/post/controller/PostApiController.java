...
public class PostApiController {
    ...
    public PostEntity create(@Valid @RequestBody PostRequest postRequest) {
        return postService.cretate(postRequest);
    }
    ...
    @GetMapping("/all")
    public Api<List<PostEntity>> list(@PageableDefault(page=0, size=10, sort = "id", direction = Sort.Direction.DESC) Pageable pageable){ // org.springframework.data.domain.Pageable // not jwt
